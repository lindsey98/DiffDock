...
def get_sequences_from_pdbfile_org(file_path):
...
def get_sequences_from_pdbfile(file_path):
    ...
    for chain in structure:
        ...
        for residue in chain:
            ...
            if {'CA', 'N', 'C'}.issubset(atom_names):
                try:
                    seq_chars.append(three_to_one[resname])
                except KeyError:
                    seq_chars.append('-')
                    unknown_aa.add(resname)
            else:
                seq_chars.append('-')

        sequence_parts.append(''.join(seq_chars))
        if unknown_aa:
            print(f"Encountered unknown AAs: {', '.join(unknown_aa)} in {file_path}. Replaced with dashes.")
...
def get_sequences(protein_files, protein_sequences):
    ...
    for i in tqdm(range(len(protein_files)), desc="Converting pdb file into sequences"):
        if protein_files[i] is not None:
            if protein_files[i] in list(seen_protein_files.keys()):
                new_sequences.append(seen_protein_files[protein_files[i]])
            else:
                sequence = get_sequences_from_pdbfile(protein_files[i])
                new_sequences.append(sequence)
                seen_protein_files[protein_files[i]] = sequence
        else:
...
def compute_ESM_singleembedding(model, alphabet, sequence):
    # settings used
    repr_layers = [33]
    truncation_seq_length = 1022

    # Convert the sequence to tensor
    _, _, toks = alphabet.get_batch_converter()([(None, sequence)])

    # Ensure only one sequence is processed
    assert toks.size(0) == 1, "Only one sequence should be processed"

    # Adjust representation layers based on the model's layers
    assert all(-(model.num_layers + 1) <= i <= model.num_layers for i in repr_layers)
    repr_layers = [(i + model.num_layers + 1) % (model.num_layers + 1) for i in repr_layers]
    ...
    with torch.no_grad():
        ...
        truncate_len = min(truncation_seq_length, len(sequence))
        embeddings = representations[33][0, 1: truncate_len + 1].clone()
        return embeddings
...
class InferenceDatasetSingleProtein(Dataset):
    def __init__(self, out_dir,
                 complex_names,
                 protein_files,
                 ligand_descriptions,
        ...
        super(InferenceDatasetSingleProtein, self).__init__()
        self.receptor_radius = receptor_radius
        self.c_alpha_max_neighbors = c_alpha_max_neighbors
        self.remove_hs = remove_hs
        self.all_atoms = all_atoms
        ...
        if lm_embeddings and (precomputed_lm_embeddings is None or precomputed_lm_embeddings[0] is None):
            ...
            lm_embeddings = compute_ESM_embeddings(model, alphabet, labels, sequences)
            lm_embedding_single = compute_ESM_singleembedding(model, alphabet, sequences[0])
            self.lm_embeddings = []
            for i in range(len(protein_sequences)):
                s = protein_sequences[i].split(':')
                self.lm_embeddings.append([lm_embeddings[f'{complex_names[i]}_chain_{j}'] for j in range(len(s))])
