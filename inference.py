...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_dir', type=str, default='./results/protein315_drugbank9k', help='Directory where the outputs will be written to')
    ...
    for idx, orig_complex_graph in tqdm(enumerate(test_loader)):
        ...
        try:
            ...
            data_list, confidence = sampling(data_list=data_list,
                                             ...
                                             no_random=True # I dont want any randomness
                                             )
            ...
            if confidence is not None and isinstance(confidence_args.rmsd_classification_cutoff, list):
                confidence = F.softmax(confidence, -1) # fixme: I add softmax to the confidence score
            ...
            write_mol_with_coords(mol_pred, pos,
                                  os.path.join(write_dir, f'rank1_confidence{confidence[0]:.2f}.sdf'))

            if args.save_visualisation:
                if confidence is not None:
                    batch_idx = re_order[0]
                    visualization_list[batch_idx].write(os.path.join(write_dir, f'rank1_reverseprocess.pdb'))
